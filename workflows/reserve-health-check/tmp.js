...
var onCronTrigger = (runtime2) => {
  ...
  let totalSupply;
  try {
    const callData = hexToBytes2("0x18160ddd");
    const supplyResult = sepoliaEvm.callContract(runtime2, {
      call: {
        to: hexToBytes2(config.stableCoinAddress),
        data: callData
      }
    });
    ...
    runtime2.log("EVM read unavailable (simulation), using fallback supply.");
  }
  let totalReserves;
  try {
    const fetchReserves = httpClient.sendRequest(runtime2, (sender) => {
    ...
    const data = json(fetchReserves());
    totalReserves = BigInt(data.totalReserves);
  } catch {
    totalReserves = 10500000n * 10n ** 18n;
    runtime2.log("HTTP unavailable (simulation), using fallback reserves.");
  }
  if (totalSupply === 0n) {
    runtime2.log("Total supply is zero. Skipping.");
    return "skipped: zero supply";
  }
  const ratioBps = totalReserves * 10000n / totalSupply;
  const compliant = ratioBps >= WARNING_RATIO;
  let status = "HEALTHY";
  if (ratioBps < PAUSE_RATIO)
    status = "EMERGENCY";
  else if (ratioBps < CRITICAL_RATIO)
    status = "CRITICAL";
  else if (ratioBps < WARNING_RATIO)
    status = "WARNING";
  runtime2.log(`Reserve Health: ratio=${ratioBps}bps status=${status}`);
  try {
    ...
    sepoliaEvm.writeReport(runtime2, {
      receiver: config.oracleAddress,
      report: report2.result()
    });
    runtime2.log("Oracle report submitted on-chain.");
  } catch {
    runtime2.log("EVM write unavailable (simulation), report not submitted.");
  }
...
function hexToBytes2(hex) {
  ...
  for (let i2 = 0;i2 < h.length; i2 += 2) {
    bytes[i2 / 2] = parseInt(h.substring(i2, i2 + 2), 16);
  }
  return bytes;
}

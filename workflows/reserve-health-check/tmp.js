...
var WARNING_RATIO = 10200n;
var CRITICAL_RATIO = 10050n;
var BREACH_RATIO = 10000n;
var onCronTrigger = (runtime2) => {
  const config = runtime2.config;
  const now = runtime2.now();
  const sepoliaEvm = new ClientCapability(ClientCapability.SUPPORTED_CHAIN_SELECTORS["ethereum-testnet-sepolia"]);
  const httpClient = new ClientCapability2;
  runtime2.log("=== StableGuard Compliance Check ===");
  const usdcData = fetchStablecoinData(runtime2, httpClient, config.defillamaUsdcEndpoint, "USDC");
  const usdtData = fetchStablecoinData(runtime2, httpClient, config.defillamaUsdtEndpoint, "USDT");
  let usdcOnChainSupply;
  try {
    const supplyResult = sepoliaEvm.callContract(runtime2, {
      call: {
        to: config.usdcSepoliaAddress,
        data: "0x18160ddd"
      }
    });
    const reply = supplyResult.result();
    usdcOnChainSupply = bytesToBigInt(reply.data);
    runtime2.log(`[EVM Read] USDC Sepolia totalSupply: ${usdcOnChainSupply}`);
  } catch {
    usdcOnChainSupply = 0n;
    runtime2.log("[EVM Read] Sepolia USDC read failed (simulation mode).");
  }
  const reportTimestamp = BigInt(Math.floor(now.getTime() / 1000));
  const proofHash = "0x" + "0".repeat(64);
  const usdcReport = computeComplianceReport(runtime2, usdcData, "USDC", reportTimestamp, proofHash);
  const usdtReport = computeComplianceReport(runtime2, usdtData, "USDT", reportTimestamp, proofHash);
  writeReportOnChain(runtime2, sepoliaEvm, config.oracleAddress, usdcReport);
  writeReportOnChain(runtime2, sepoliaEvm, config.oracleAddress, usdtReport);
  ...
  runtime2.log(`=== Result: ${summary} ===`);
  return summary;
};
function fetchStablecoinData(runtime2, httpClient, endpoint, symbol2) {
  try {
    const fetchResult = httpClient.sendRequest(runtime2, (sender) => {
      const resp = sender.sendRequest({ url: endpoint, method: "GET" });
      return resp.result();
    }, consensusIdenticalAggregation());
    const body = json(fetchResult());
    ...
    const permittedAssetsOnly = symbol2 === "USDC";
    const noRehypothecation = true;
    const now = Math.floor(Date.now() / 1000);
    const lastAuditTimestamp = symbol2 === "USDC" ? BigInt(now - 15 * 86400) : BigInt(now - 45 * 86400);
    runtime2.log(`[HTTP] ${symbol2}: supply=$${circulatingUsd.toLocaleString()}, permittedAssets=${permittedAssetsOnly}`);
    return { totalReserves, totalSupply, permittedAssetsOnly, noRehypothecation, lastAuditTimestamp };
  } catch (e) {
    runtime2.log(`[HTTP] ${symbol2} fetch failed: ${e}. Using fallback.`);
    const fallback = symbol2 === "USDC" ? 45000000000n : 140000000000n;
    return {
      totalReserves: fallback * 10n ** 18n,
      totalSupply: fallback * 10n ** 18n,
      permittedAssetsOnly: symbol2 === "USDC",
      noRehypothecation: true,
      lastAuditTimestamp: 0n
    };
  }
}
function computeComplianceReport(runtime2, data, symbol2, timestamp, proofHash) {
  if (data.totalSupply === 0n) {
    runtime2.log(`[Compute] ${symbol2}: zero supply, skipping.`);
    return { ratioBps: 0, status: "SKIPPED", compliant: false, encodedReport: "0x" };
  }
  const ratioBps = Number(data.totalReserves * 10000n / data.totalSupply);
  let status = "HEALTHY";
  if (ratioBps < Number(BREACH_RATIO))
    status = "BREACH";
  else if (ratioBps < Number(CRITICAL_RATIO))
    status = "CRITICAL";
  else if (ratioBps < Number(WARNING_RATIO))
    status = "WARNING";
  const auditOverdue = data.lastAuditTimestamp > 0n && timestamp - data.lastAuditTimestamp > 30n * 86400n;
  const compliant = ratioBps >= Number(WARNING_RATIO) && data.permittedAssetsOnly && data.noRehypothecation && !auditOverdue;
  runtime2.log(`[Compute] ${symbol2}: ratio=${ratioBps}bps status=${status} compliant=${compliant}`);
  if (!data.permittedAssetsOnly)
    runtime2.log(`  WARNING: ${symbol2}: Non-permitted asset types in reserves`);
  if (!data.noRehypothecation)
    runtime2.log(`  WARNING: ${symbol2}: Possible rehypothecation detected`);
  if (auditOverdue)
    runtime2.log(`  WARNING: ${symbol2}: Audit overdue (>30 days)`);
  const complianceScore = computeComplianceScore(ratioBps, data.permittedAssetsOnly, data.noRehypothecation, data.lastAuditTimestamp, timestamp);
  const grade = complianceScore >= 80 ? "COMPLIANT" : complianceScore >= 50 ? "AT_RISK" : "NON_COMPLIANT";
  runtime2.log(`[Score] ${symbol2}: ${complianceScore}/100 (${grade})`);
  const symbolBytes = stringToBytes4Hex(symbol2);
...
function writeReportOnChain(runtime2, evm, oracleAddress, report2) {
  if (report2.status === "SKIPPED")
    return;
  try {
    const reportResponse = runtime2.report({
      encodedPayload: hexToBase64(report2.encodedReport),
      encoderName: "evm",
      signingAlgo: "ecdsa",
      hashingAlgo: "keccak256"
    }).result();
    const writeResult = evm.writeReport(runtime2, {
    ...
    if (writeResult.txStatus === TxStatus.SUCCESS) {
      ...
      runtime2.log(`[EVM Write] Report submitted. Tx: ${txHash}`);
    } else {
      runtime2.log(`[EVM Write] Report failed: ${writeResult.txStatus}`);
    }
  } catch (e) {
    runtime2.log(`[EVM Write] Unavailable (simulation): ${e}`);
  }
}
function computeComplianceScore(ratioBps, permittedAssets, noRehypo, lastAuditTs, nowTs) {
...
function stringToBytes4Hex(str) {
  const padded = str.padEnd(4, "\x00");
  let hex = "0x";
  for (let i2 = 0;i2 < 4; i2++) {
    hex += padded.charCodeAt(i2).toString(16).padStart(2, "0");
  }
  return hex;
}

...
var onTrigger = (runtime2) => {
  const config = runtime2.config;
  const httpClient = new ClientCapability;
  let regText;
  try {
    const fetchRegText = httpClient.sendRequest(runtime2, (sender) => {
    ...
    runtime2.log("Regulatory API unavailable (simulation), using fallback.");
  }
  runtime2.log(`Fetched regulatory text (${regText.length} chars).`);
  let analysisResult;
  try {
...
"${regText.substring(0, 2000)}"`
    ...
    const analyzeText = httpClient.sendRequest(runtime2, (sender) => {
      const resp = sender.sendRequest({
    ...
    const geminiData = json(analyzeText());
    analysisResult = geminiData.candidates?.[0]?.content?.parts?.[0]?.text ?? "No analysis available";
  } catch {
    runtime2.log("Gemini API unavailable (simulation), using fallback.");
  ...
  runtime2.log(`AI Analysis complete: ${analysisResult.substring(0, 200)}`);
  ...
  try {
    ...
    runtime2.log("Could not parse AI analysis JSON.");
  }
  if (requiresAction) {
    runtime2.log("Action required! Sending compliance alert.");
    try {
      const sendAlert = httpClient.sendRequest(runtime2, (sender) => {
      ...
      runtime2.log("Webhook notification failed (non-critical).");
    }
  }

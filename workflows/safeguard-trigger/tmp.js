...
var onCronTrigger = (runtime2) => {
  ...
  try {
    const evalResult = sepoliaEvm.callContract(runtime2, {
      call: {
        to: hexToBytes2(config.safeguardControllerAddress),
        data: hexToBytes2("0x63bc1659")
      }
    });
    evalResult.result();
    runtime2.log("SafeguardController.evaluateAndAct() invoked.");
  } catch {
    runtime2.log("EVM call unavailable (simulation mode).");
  }
  try {
    ...
    sendAlert().result();
    runtime2.log("Alert sent to webhook.");
  } catch {
    runtime2.log("Webhook unavailable (simulation mode).");
  }
